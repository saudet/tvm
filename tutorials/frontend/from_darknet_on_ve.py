...
# under the License.
"""
Compile YOLO-V2 and YOLO-V3 in DarkNet Models (for TV target)
=============================================================
...
with tvm.transform.PassContext(opt_level=3, config={"tir.disable_vectorize": True}):
    compiled_lib = relay.build(mod, target="llvm -mtriple=ve-linux -mattr=-vpu", params=params)
compiled_lib.export_library("lib/libdarknetve.so", cc.cross_compiler("/opt/nec/ve/bin/nc++"))
...
tvm.runtime.load_module("lib/libdarknetve.so", "vepreload")
tvm.runtime.load_module("lib/libtvm_runtime_pack.so", "ve")
loaded_lib = tvm.runtime.load_module("lib/libdarknetve.so", "ve")
m = graph_runtime.GraphModule(loaded_lib["default"](ctx))

# set inputs
m.set_input("data", tvm.nd.array(data.astype(dtype), ctx))
# execute
